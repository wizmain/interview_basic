...
- (void)requestJobKoreaCategory:(NSString *)catType category:(NSString *)category {
    
    [UIApplication sharedApplication].networkActivityIndicatorVisible = YES;
    
    NSString *urlString = [NSString stringWithFormat:@"%@%@?cat_type=%@&cat=%@", kServerUrl, kJobKoreaCategoryUrl, catType, category];
    NSLog(@"urlString=%@", urlString);
    NSURL *url = [NSURL URLWithString:urlString];
    NSURLRequest *request = [NSURLRequest requestWithURL:url];
    NSMutableArray *resultList = [[NSMutableArray alloc] init];
    ...
    AFJSONRequestOperation *operation = [AFJSONRequestOperation JSONRequestOperationWithRequest:request success:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON) {
        NSLog(@"requestJobKoreaCategory Success %@", JSON);
        [UIApplication sharedApplication].networkActivityIndicatorVisible = NO;
        ...
        if ([delegate respondsToSelector:@selector(bindJobKoreaCategory:)]) {
            [delegate bindJobKoreaCategory:resultList];
        }
