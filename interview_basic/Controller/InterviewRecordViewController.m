...
@property (nonatomic, retain) IBOutlet UITextView *categoryTitleText;
@property (nonatomic, retain) IBOutlet UITextView *questionTitleText;
...
- (void)viewDidLoad
{
    ...
    */
    ...
    if (self.questionList) {
        _totalQuestion = self.questionList.count;
...
- (void)viewDidUnload
{
    ...
    self.questionTitleText = nil;
    self.categoryTitleText = nil;
}
...
- (void)viewDidDisappear:(BOOL)animated
{
    ...
    [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(onTimer:) object:nil];//performSelector인 경우
}
...
- (BOOL)shouldAutorotateToInterfaceOrientation:(UIInterfaceOrientation)interfaceOrientation{
    return (interfaceOrientation == UIDeviceOrientationLandscapeRight || interfaceOrientation == UIInterfaceOrientationMaskPortrait);
...
- (void)willAnimateRotationToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration {
    if(toInterfaceOrientation == UIInterfaceOrientationLandscapeRight){
        ...
        if (IS_iPhone_5) {
            self.captureVideoPreviewLayer.frame = CGRectMake(0, 0, 568, 276);
            
            self.categoryTitleText.frame = CGRectMake(0, 0, 469, 33);
            self.questionTitleText.frame = CGRectMake(470, 0, 98, 33);
        } else {
            //self.videoPreviewView.frame = CGRectMake(0, 0, 480, 300);
            self.captureVideoPreviewLayer.frame = CGRectMake(0, 0, 480, 276);
            
            self.categoryTitleText.frame = CGRectMake(0, 0, 381, 33);
            self.questionTitleText.frame = CGRectMake(382, 0, 98, 33);
        }
        
        self.spinner.frame = CGRectMake(220, 130, 37, 37);
        
        //self.questionTitleBarButton
        ...
        if(IS_iPhone_5) {
            self.captureVideoPreviewLayer.frame = CGRectMake(0, 0, 320, 524);
        } else {
            //self.videoPreviewView.frame = CGRectMake(0, 0, 320, 490);
            self.captureVideoPreviewLayer.frame = CGRectMake(0, 0, 320, 436);
        }
        
        self.spinner.frame = CGRectMake(142, 199, 37, 37);
        self.categoryTitleText.frame = CGRectMake(0, 0, 221, 33);
        self.questionTitleText.frame = CGRectMake(222, 0, 98, 33);
        
    } else if(toInterfaceOrientation == UIInterfaceOrientationLandscapeLeft) {
        [self.captureVideoPreviewLayer setOrientation:AVCaptureVideoOrientationLandscapeLeft];
...
- (void)initToolbarButtons {
    ...
    if(self.settings != nil){
        _defaultAnswerTime = [[NSString stringWithFormat:@"%@",[self.settings answerTerm]] intValue];
    } else {
        _defaultAnswerTime = 25;
    }
    ...
    self.questionTitleText.text = [NSString stringWithFormat:@"질문:0/9"];
    ...
    */
    if ([self captureManager] == nil) {
		...
		if ([[self captureManager] setupSession]) {
            ...
            if(IS_iPhone_5) {
                newCaptureVideoPreviewLayer.frame = CGRectMake(0, 0, 320, 524);
            } else {
                //self.videoPreviewView.frame = CGRectMake(0, 0, 320, 490);
                newCaptureVideoPreviewLayer.frame = CGRectMake(0, 0, 320, 436);
            }
...
- (void)playQuestion:(int) qNo {
    ...
    if ([[[self captureManager] recorder] isRecording]) {
        ...
        if(qNo <= _totalQuestion){
            ...
            //질문자막표시
            NSString *qText = [NSString stringWithFormat:@"%@",[q valueForKey:@"Q_TITLE"]];
            [self.questionText setText:qText];
            //질문 박스 높이 조정 글자 사이즈에 따라서
            CGSize textSize = [qText sizeWithFont:[UIFont systemFontOfSize:16.0f] constrainedToSize:CGSizeMake((self.questionText.frame.size.width - 20) - PADDING * 3, 1000.0f)];
            CGRect qFrame = self.questionText.frame;
            int addHeight = qFrame.size.height;
            if(textSize.height > qFrame.size.height) {
                addHeight = textSize.height - qFrame.size.height;
                qFrame.size.height = textSize.height;
                qFrame.origin.y = qFrame.origin.y - addHeight;
            }
            
            self.questionText.frame = qFrame;
            ...
            if([Utils isNullString:[NSString stringWithFormat:@"%@",[q objectForKey:@"ELAPSED_TIME"]]]){
                ...
                if([Utils isNumbericString:[NSString stringWithFormat:@"%@",[q objectForKey:@"ELAPSED_TIME"]]]){
                    ...
                    self.elapsedTime = [[NSString stringWithFormat:@"%@",[q objectForKey:@"ELAPSED_TIME"]] intValue];
                } else {
                    self.elapsedTime = self.defaultAnswerTime;
                }
            ...
            self.questionTitleText.text = [NSString stringWithFormat:@"질문:%d/%d", qNo,[self.questionList count]];
            ...
            //[s release];
            //질문타이머 시작
            if(_currentQuestion > _totalQuestion){//현재 질문이 질문갯수 넘어갔을때 해제
                if(self.timer)
                    [self.timer invalidate];
            } else {
                //[self performSelector:@selector(timerStart) withObject:nil afterDelay:4];//타이머로..
                [self performSelector:@selector(onTimer:) withObject:nil afterDelay:5];//다른방식으로
            }
        }
    }
}
...
- (void)onTimer:(NSTimer *)timer {
    ...
    if(self.elapsedTime > 0){
        self.elapsedTime--;
        [self.answerTimeInfoBarButton setTitle:[NSString stringWithFormat:@"%d초",self.elapsedTime]];
        [self performSelector:@selector(onTimer:) withObject:nil afterDelay:1];
    } else {
...
- (void)bindQuestionData:(NSMutableArray *)questionData {
    ...
    if(self.questionList){
        self.totalQuestion = [self.questionList count];
    } else {
...
- (void)insertInterviewData:(NSString *)fileUrl {
    ...
    if(self.managedObjectContext != nil) {
        ...
        if(self.isClose) {//창을 닫기 위해 닫는 경우
            // 원상복귀
