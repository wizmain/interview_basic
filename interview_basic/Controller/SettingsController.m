...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([indexPath section] == 0) {
        ...
        LoginProperties *loginProp = [Utils loginProperties];
        cell.textLabel.text = @"로그인";
        cell.detailTextLabel.text = loginProp.userID;

    } else if ([indexPath section] == 1) {
	
        cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
        NSString *answerTermStr = @"30";
        if (self.settings) {
            if ([Utils isNullString:self.settings.answerTerm]) {
                
            } else {
                answerTermStr = self.settings.answerTerm;
            }
        }
        cell.textLabel.text = [NSString stringWithFormat:@"면접답변시간 : %@초", answerTermStr];
...
- (void)tableView:(UITableView *)tv didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if([indexPath section] == 0){
        if ([indexPath row] == 0) {
            LoginSettingViewController *loginSetting = [[LoginSettingViewController alloc] initWithNibName:@"LoginSettingViewController" bundle:nil];
